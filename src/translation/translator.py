...
class BookTranslator:
    ...
    def __init__(
        ...
        self.progress_tracker = ProgressTracker(progress_file) if progress_file else None
    ...
    def translate_book(
        ...
        print(f"📖 Processing chapters {from_chapter} to {min(to_chapter, total_chapters)}")
            ...
            for item in chapters:
                if from_chapter <= current_chapter <= to_chapter:
                    ...
                    if self.progress_tracker:
                        start_chunk = self.progress_tracker.get_chapter_progress(current_chapter)
    ...
    def _translate_chapter(
        ...
        translated_chunks = [""] * start_chunk  # Placeholders for already translated chunks
            ...
            try:
                ...
                if "rate limit" in error_msg or "quota" in error_msg:
                    if attempt < self.max_retries - 1:
                        print(f"    ⏳ Rate limit hit. Waiting {self.retry_delay}s... (attempt {attempt + 1}/{self.max_retries})")
                ...
                raise TranslationError(f"Translation failed after {self.max_retries} attempts: {e}")
