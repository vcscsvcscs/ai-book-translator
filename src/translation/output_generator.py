...
class OutputGenerator:
    ...
    def generate_outputs(self, original_book, output_path, from_lang, to_lang, output_formats):
        """Generate all requested output formats."""
        base_path = Path(output_path).with_suffix("")
        for format_type in output_formats:
            if format_type == "markdown":
                self._generate_markdown(base_path.with_suffix(".md"), from_lang, to_lang)
            elif format_type == "epub":
                self._generate_epub(original_book, base_path.with_suffix(".epub"), from_lang, to_lang)
            else:
