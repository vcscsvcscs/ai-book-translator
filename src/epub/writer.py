...
class EPUBWriter:
    ...
    def update_metadata(self, metadata: Dict[str, Any]) -> "EPUBWriter":
        ...
        if not self._book:
            raise EpubError(
                "No book initialized. Call create_new_book() or create_from_reader() first."
            )
    ...
    def update_chapter_content(
        self, chapter_number: int, new_content: str
    ) -> "EPUBWriter":
        """
    ...
    def add_chapter(
        self, title: str, content: str, file_name: Optional[str] = None
    ) -> "EPUBWriter":
        """
        ...
        chapter = epub.EpubHtml(
            title=title, file_name=file_name, lang=self._book.language
        )
    ...
    def reorder_chapters(self, new_order: List[int]) -> "EPUBWriter":
        ...
        for spine_item in self._book.spine:
            if spine_item[0] in [item.get_id() for item in document_items]:
                new_spine.append(
                    (reordered_items[document_index].get_id(), spine_item[1])
                )
                document_index += 1
            else:
        ...
        try:
            ...
            if create_backup and self.output_path.exists():
                backup_path = self.output_path.with_suffix(
                    f"{self.output_path.suffix}.backup"
                )
    ...
    def save_partial(self, suffix: str = ".partial") -> Path:
        ...
        partial_path = self.output_path.with_name(
            f"{self.output_path.stem}{suffix}{self.output_path.suffix}"
        )
    ...
    def get_book_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "title": self._book.get_metadata("DC", "title")[0][0]
                if self._book.get_metadata("DC", "title")
                else "Unknown",
                "author": self._book.get_metadata("DC", "creator")[0][0]
                if self._book.get_metadata("DC", "creator")
                else "Unknown",
                "language": self._book.get_metadata("DC", "language")[0][0]
                if self._book.get_metadata("DC", "language")
                else "Unknown",
