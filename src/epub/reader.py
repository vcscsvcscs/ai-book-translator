...
class EPUBReader:
    ...
    def _extract_metadata(self) -> Dict[str, str]:
        ...
        # Basic metadata
        metadata["title"] = (
            self._book.get_metadata("DC", "title")[0][0]
            if self._book.get_metadata("DC", "title")
            else "Unknown"
        )
        metadata["author"] = (
            self._book.get_metadata("DC", "creator")[0][0]
            if self._book.get_metadata("DC", "creator")
            else "Unknown"
        )
        metadata["language"] = (
            self._book.get_metadata("DC", "language")[0][0]
            if self._book.get_metadata("DC", "language")
            else "Unknown"
        )
        metadata["publisher"] = (
            self._book.get_metadata("DC", "publisher")[0][0]
            if self._book.get_metadata("DC", "publisher")
            else "Unknown"
        )
            ...
            if not chapters:
                ...
                for chapter in chapters:
                    ...
                    try:
                        ...
                        warnings.append(
                            f"Chapter {chapter.chapter_number} has invalid HTML"
                        )
